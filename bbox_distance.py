...
for jsonlabel in jsonlabels:#循环处理每一个json
    ...
    while (count < length):#处理每一个目标
        ...
        distance = im.getpixel((testx,testy))
        distance = format(float(distance)/65535.0*20.0, '.2f')
        setting[count]['testpoint'] = {'x': testx, 'y': testy,'distance': distance }
        print (label,xmax,xmin,ymax,ymin,testx,testy,distance)
        ...
        cv2.putText(img, label+'_'+str(distance), (int(xmin), int(ymax)-12), 0, 1e-3 * h , (0, 0, 255),thick//3) #文字1，1分别是字体字号
        count+=1
    cv2.imwrite('/Users/apple/darkflow/ZED1/depth/outputs/'+file_name +'.png',img)#输出
